...
export async function GET(req: Request , res: Response) {
    ...
    try {
        ...
        if (!subscription) {
            return NextResponse.json({ message: 'Aucune souscription trouvée pour cet utilisateur' }, { status: 404 });
        }
      return NextResponse.json({ message: 'Souscriptions récupérée avec succès', data: subscription }, { status: 200 });
}
