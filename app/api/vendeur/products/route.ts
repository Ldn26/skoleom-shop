...
export async function GET(request: Request) {
  ...
  try {
    const raw = await wooService.getProductsRaw({
      per_page: 100,
      status: 'any',
      _fields: 'id,name,slug,status,price,stock_status,images,meta_data',
    });
    const list: WooProductRaw[] = Array.isArray(raw) ? (raw as WooProductRaw[]) : [];

    const owned = list.filter((p) =>
      (p.meta_data ?? []).some((m) => m.key === '_monetizer_user_id' && String(m.value) === mine),
    );
    ...
    return NextResponse.json({
      ...
      meta: {
        ...
        page,
        per_page: perPage,
        total_pages: Math.max(1, Math.ceil(owned.length / perPage)),
      },
    });
  } catch (err: unknown) {
