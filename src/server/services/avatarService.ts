...
type ImageParts = { mimeType: string; base64: string };
...
type AvatarRecord = {
  ...
  twin: ImageParts | null;
  measurements: unknown;
  analysis: unknown;
};

export const avatarCache = new Map<string, AvatarRecord>();
...
export function toDataUrl({ mimeType, base64 }: ImageParts): string {
  return `data:${mimeType};base64,${base64}`;
}

export function normalizeMeasurements(m: Measurements = {}) {
  return {
    height: m.height !== null ? Number(m.height) : null,
    weight: m.weight !== null ? Number(m.weight) : null,
    chest: m.chest !== null ? Number(m.chest) : null,
...
export async function destroyFromCloudinary(publicId?: string | null) {
  ...
  try {
    ...
    console.warn('[cloudinary destroy]', publicId, (err as Error).message);
  }
}

export async function fetchImageAsBase64(url: string): Promise<ImageParts> {
  const { data, headers } = await axios.get(url, { responseType: 'arraybuffer', timeout: 30000 });
...
export async function analyzePhoto({ mimeType, base64 }: ImageParts): Promise<unknown> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) throw new Error('GEMINI_API_KEY manquant');
...
export async function generateTwinImage(
  { mimeType, base64 }: ImageParts,
  measurements: Measurements,
): Promise<ImageParts | null> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) throw new Error('GEMINI_API_KEY manquant');
...
Ne renvoie QUE l'image.`;
  ...
  try {
    ...
    console.warn('[gemini twin]', e.response?.status || '', e.message);
    return null;
  }
}
...
export async function getAvatar(avatarUuid: string): Promise<AvatarRecord | null> {
  const cached = avatarCache.get(avatarUuid);
  if (cached && cached.twin && cached.id !== null) return cached;
  ...
  let twin: ImageParts | null = null;
  try {
    twin = await fetchImageAsBase64(row.avatar_url);
  } catch (e) {
    console.warn('[getAvatar twin]', (e as Error).message);
  }

  let photo: ImageParts | null = null;
  try {
    photo = await fetchImageAsBase64(row.original_url);
  } catch (e) {
    console.warn('[getAvatar original]', (e as Error).message);
  }
