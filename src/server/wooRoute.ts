...
import { wooService as wooServiceBase } from '@/server/services/wooService';
import { wpService as wpServiceBase } from '@/server/services/wpService';
...
export async function run(fn: () => Promise<unknown>): Promise<Response> {
  try {
...
export function buildParams(q: Record<string, unknown>) {
  const { page, limit, per_page, ...rest } = q;
  const p = Math.max(1, Number(page) || 1);
  const pp = Math.max(1, Number(limit ?? per_page) || 50);
  return { ...rest, page: p, paged: p, per_page: pp, limit: pp, offset: (p - 1) * pp };
}

export function injectUserMeta<T extends BodyWithMetaData>(body: T, userId?: number | string): T {
  ...
  return {
    ...body,
    meta_data: [...existing, { key: '_monetizer_user_id', value: String(userId) }],
  };
}
...
async function createUser({ username, email, password }: CreateUserInput) {
  ...
  try {
    const { data } = await wooUserClient.post<WooCustomer>('customers', {
      email,
      username: login,
      password,
    });
    ...
    if (axios.isAxiosError(error)) {
      ...
      if (data?.code === 'registration-error-email-exists') {
        const { data: found } = await wooUserClient
          .get<WooCustomer[]>('customers', { params: { email } })
          .catch(() => ({ data: [] }));
...
export const wooService = wooServiceBase;
