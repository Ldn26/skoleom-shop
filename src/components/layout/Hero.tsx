...
//       <div className="relative z-10 mx-auto flex w-full max-w-[1600px] px-5 sm:px-8 lg:px-12">
...
export default function Hero() {
  ...
  return (
    <section className="sk-hero relative mt-8 flex min-h-[100svh] w-full items-center overflow-hidden">
      <style>{`
        .sk-hero [data-reveal]{opacity:0;transform:translateY(18px);animation:sk-hero-rise .8s cubic-bezier(.16,.84,.28,1) forwards;}
        @keyframes sk-hero-rise{to{opacity:1;transform:none;}}
        @media (prefers-reduced-motion: reduce){.sk-hero [data-reveal]{animation:none!important;opacity:1!important;transform:none!important;}}
      ...
      <img
        ...
        alt=""
        aria-hidden
        className="absolute inset-0 hidden h-full w-full object-cover object-[70%_center] sm:object-right md:block"
        onError={(e) => {
          e.currentTarget.style.display = 'none';
        }}
      />
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-[1600px] px-5 sm:px-8 lg:px-12">
        <div className="flex max-w-xl flex-col justify-center py-24 text-left lg:max-w-2xl">
          ...
          <h1
            data-reveal
            style={{ animationDelay: '0.16s' }}
            className="font-display text-4xl uppercase leading-[0.9] tracking-tight text-white sm:text-6xl lg:text-7xl"
          >
            {isBuyer ? (
              <>
