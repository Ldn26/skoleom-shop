...
//     <button
//       onClick={onChange}
//       className={`group flex w-full items-center justify-between gap-3 rounded-xl px-3 text-left transition-colors duration-150 ${
//         checked ? 'text-lime-400' : 'text-zinc-300 hover:bg-white/[0.05]'
//       } ${indent ? 'py-2 text-sm' : 'py-2.5'}`}
//     >
//       <span className="flex items-center gap-3">
//         <span
//           className={`grid h-[18px] w-[18px] place-items-center rounded-[6px] border transition-all duration-150 ${
//             checked ? 'border-lime-400 bg-lime-400 text-black' : 'border-white/20 group-hover:border-white/40'
//           }`}
//         >
...
//   // children lookup, built once from the full in-memory list
//   const byParent = useMemo(() => {
//     const map = new Map<number, TaxonomyItem[]>();
//     all.forEach((c) => {
//       const arr = map.get(c.parent ?? 0) ?? [];
//       arr.push(c);
//       map.set(c.parent ?? 0, arr);
//     });
//     return map;
//   }, [all]);

//   // top-level = children of Skoleom Shop (fallback to parent 0 if not nested)
//   const topLevel = useMemo(() => {
//     let roots = byParent.get(ROOT_CATEGORY_ID) ?? [];
//     if (roots.length === 0) roots = (byParent.get(0) ?? []).filter((c) => c.id !== ROOT_CATEGORY_ID);
//     const q = catQuery.trim().toLowerCase();
//     return q ? roots.filter((c) => c.name.toLowerCase().includes(q)) : roots;
//   }, [byParent, catQuery]);
...
//     selectedCategory === slug ? clearCategory() : setCategory(slug);
//   };
...
//                       <CheckRow label={cat.name} count={cat.count} checked={isActive} onChange={() => toggleMain(cat.id, cat.slug)} />
//                     </div>
//                     {subs.length > 0 && (
//                       <button
...
//                             indent
//                             onChange={() => (selectedCategory === sub.slug ? clearCategory() : setCategory(sub.slug))}
//                           />
//                         ))}
//                       </div>
//                     </div>
      ...
      <Section title="Catégories">
        <div className="relative mb-3">
          ...
          <input
            ...
            onChange={(e) => setCatQuery(e.target.value)}
            placeholder="Rechercher une catégorie"
            className="w-full rounded-xl border border-white/10 bg-white/[0.03] py-2.5 pl-9 pr-3 text-sm text-white outline-none placeholder:text-zinc-600 focus:border-lime-400/50"
          />
        </div>
