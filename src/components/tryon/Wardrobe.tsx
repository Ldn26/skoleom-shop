    ...
    <div className="bg-white/[0.03] border border-white/10 rounded-2xl p-5 flex flex-col gap-4">
      {!avatarReady && (
        <div className="flex items-center gap-2 bg-white/[0.03] border border-white/10 rounded-[10px] px-3 py-2">
          <svg
            className="w-4 h-4 text-[#8a93a8] flex-shrink-0"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            strokeWidth="1.8"
      ...
      {isLoading &&
        Array.from({ length: 4 }).map((_, i) => (
          <div
            key={i}
            className="flex gap-3 p-[9px] border border-white/10 rounded-[13px] animate-pulse"
          >
      ...
      {noFilterMatch && (
        <div className="text-center py-6">
          ...
          <button
            ...
            className="mt-2 text-[11px] text-[rgb(163_230_53)] hover:underline"
          >
